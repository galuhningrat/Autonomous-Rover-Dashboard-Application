...
MainWindow::MainWindow(QWidget *parent)
    ...
    , batteryCapacity(222000.0) // 222 Wh * 1000
    , totalPowerConsumption(0.0)
    , powerSavingMode(false)
{
    ui->setupUi(this);

    // Initialize sensor power table
    initializeSensorPowerTable();
    ...
    // Setup log file
    logFile = new QFile("sensor_log.csv");
    if (logFile->open(QIODevice::WriteOnly | QIODevice::Text)) {
        logStream = new QTextStream(logFile);
        *logStream << "Time,Bus Voltage,Shunt Voltage,Load Voltage,Current,Power\n";
    } else {
        qDebug() << "Failed to open log file.";
    }

    // Connect power saving mode toggle
    connect(ui->powerSavingModeCheckBox, &QCheckBox::toggled, this, &MainWindow::togglePowerSavingMode);
}
...
void MainWindow::processSensorPowerData(const QString &data) {
    QStringList parts = data.split(',');
    if (parts.size() == 7) {
        updateSensorPowerTable(parts);
        totalPowerConsumption = parts[6].toFloat();
        updateRemainingOperationTime();
    }
}
...
void MainWindow::updateBatteryProgressBar(float power) {
    ...
    QString colorStyle;
    if (percentage > 50) {
        colorStyle = "QProgressBar::chunk { background-color: #4CAF50; }"; // Green
    } else if (percentage > 20) {
        colorStyle = "QProgressBar::chunk { background-color: #FFC107; }"; // Yellow
    } else {
        colorStyle = "QProgressBar::chunk { background-color: #F44336; }"; // Red
    }

    ui->persentase->setStyleSheet(ui->persentase->styleSheet() + colorStyle);
...
void MainWindow::on_setBatteryCapacityButton_clicked() {
    bool ok;
    float newCapacity = QInputDialog::getDouble(this, tr("Set Battery Capacity"),
                                                tr("Enter battery capacity (Wh):"), batteryCapacity / 1000.0, 0, 10000, 2, &ok);
    if (ok) {
        batteryCapacity = newCapacity * 1000.0; // Convert Wh to mWh
        updateRemainingOperationTime();
        arduino->write(QString("SET_BATTERY_CAPACITY:%1\n").arg(newCapacity).toUtf8());
    }
}

void MainWindow::updateRemainingOperationTime() {
    if (totalPowerConsumption > 0) {
        float remainingTime = batteryCapacity / totalPowerConsumption; // in hours
        int hours = static_cast<int>(remainingTime);
        int minutes = static_cast<int>((remainingTime - hours) * 60);
        ui->remainingTimeLabel->setText(QString("%1h %2m").arg(hours).arg(minutes));
    } else {
        ui->remainingTimeLabel->setText("N/A");
    }
}

void MainWindow::togglePowerSavingMode(bool enabled) {
    powerSavingMode = enabled;
    if (powerSavingMode) {
        // Implement power saving measures
        if (autoMode) {
            autoTimer->setInterval(100); // Slow down servo movement
        }
        // Reduce other sensor update frequencies if applicable
    } else {
        // Restore normal operation
        if (autoMode) {
            autoTimer->setInterval(50);
        }
        // Restore other sensor update frequencies if applicable
    }
}

void MainWindow::initializeSensorPowerTable() {
    ui->sensorPowerTable->setColumnCount(2);
    ui->sensorPowerTable->setHorizontalHeaderLabels({"Sensor", "Power (mW)"});
    ui->sensorPowerTable->setRowCount(6);
    ui->sensorPowerTable->setItem(0, 0, new QTableWidgetItem("HC-SR04"));
    ui->sensorPowerTable->setItem(1, 0, new QTableWidgetItem("HW-493"));
    ui->sensorPowerTable->setItem(2, 0, new QTableWidgetItem("Servo Motor"));
    ui->sensorPowerTable->setItem(3, 0, new QTableWidgetItem("INA219"));
    ui->sensorPowerTable->setItem(4, 0, new QTableWidgetItem("Arduino"));
    ui->sensorPowerTable->setItem(5, 0, new QTableWidgetItem("Total"));
}

void MainWindow::updateSensorPowerTable(const QStringList &powerData) {
    for (int i = 0; i < 6; ++i) {
        ui->sensorPowerTable->setItem(i, 1, new QTableWidgetItem(powerData[i+1] + " mW"));
    }
}
...
void MainWindow::logData() {
    if (logStream) {
        QDateTime currentTime = QDateTime::currentDateTime();
        *logStream << currentTime.toString("yyyy-MM-dd hh:mm:ss") << ","
                   << ui->busVoltageLabel->text().split(" ")[0] << ","
                   << ui->shuntVoltageLabel->text().split(" ")[0] << ","
                   << ui->loadVoltageLabel->text().split(" ")[0] << ","
                   << ui->currentLabel->text().split(" ")[0] << ","
                   << ui->powerLabel->text().split(" ")[0] << "\n";
        logStream->flush();
    }
}
